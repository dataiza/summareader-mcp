...
#
#   scripts/appimage/build.sh <bundle-dir> <frozen-mirror> <version> <out.AppImage>
#
...
mirror="${2:?the frozen summareader-mcp}"
version="${3:?the version}"
out="${4:?where to write the AppImage}"

here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo="$(cd "$here/../.." && pwd)"
id=sk.dataiza.summareader_mcp_console
...
# Beside the console's own executable, which is where launcher() looks after
# the environment — see console/lib/src/mirror.dart. Inside an AppImage that
# resolves into the mount, so the window finds its mirror without knowing it is
